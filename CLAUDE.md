...
2. Run `npm run render:last-month` → writes `exports/monthly-YYYY-MM.mp4`
...
```sh
npm run render:last-month       # primary command — renders last month's reel
...
MONTH=2026-04 npm run render:last-month  # target a specific month
npm run render:each-month                # one video per month found in gpx/
npm run render:all                       # every activity in gpx/, no month filter
                                         # → exports/all-YYYY-MM-DD-to-YYYY-MM-DD.mp4
npm run render:draft            # 15fps quick preview
```

Key env vars for `render:last-month` / `render:all`:
...
- Card text comes from the span of the loaded routes (`routesPeriodLabel`): one month gives "My Month in Fitness" / "July 2026", several give "My Fitness" / "January 2026 to August 2026". The renderer only passes `kicker`/`endTitle` when it has a `MONTH`, so an unfiltered `render:all` labels itself from the data.
...
Before rendering, `render-instagram.mjs` compares each route's coordinates against the area bounds in `basemap/basemap.json` and runs `build:basemap` itself if anything is uncovered — a gap would otherwise render over blank land at z14 (the world extract stops at z5), which is only visible after the whole render. It refuses to render if the fetch fails or leaves a gap. `render:each-month` covers every month up front rather than discovering a gap eight renders in.

`fetch-basemap.mjs` is incremental: extracts already on disk are kept, a cluster already inside an existing area is skipped, and one that only partly overlaps is widened to the union and refetched (the old file is deleted) so overlapping extracts never draw the same labels twice. `FORCE_BASEMAP_REBUILD=1` ignores what exists and rebuilds everything.
